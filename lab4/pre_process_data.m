...
listp = {'P000a1','P001a1','P002a1','P003a1','P004a1',
         'P005a1','P006a1','P007a1','P008a1','P009a1'};
% Resampling.
h = zeros(Nout, K);
p = h;
for k = 1 : K
...
Z = h(:, ktrain); [NR, NC] = size(Z); Z = reshape(Z, NR*NC, 1);
[~, C] = kmeans(Z, Kquant); C = sort(C);  % C stores the centroids
% Perform quantization on all the signals.
hq = cell(1, K);  % K
for k = 1 : K
    hk = h(:, k)'; hkM = kron(hk, ones(Kquant, 1));
    cM = kron(ones(1, NR), C); b = abs(hkM - cM); [~, I] = min(b, [], 1);
    hq{k} = I;
end
% ----------------PD patients
% Find the centroids for the training signals.
Z = p(:, ktrain); [NR, NC] = size(Z); Z = reshape(Z, NR*NC, 1);
[~, C] = kmeans(Z, Kquant); C = sort(C);  %C stores the centroids
% Perform quantization on all the signals.
pq = cell(1, K);
for k = 1 : K
    pk = p(:, k)'; pkM = kron(pk, ones(Kquant, 1));
    cM = kron(ones(1, NR), C); b = abs(pkM - cM); [~, I] = min(b, [], 1);
    pq{k} = I;
end
