...
class SolveMinProbl(object):
    ...
    def __init__(self,y,A,yval,Xval):
        self.matr = A
...
class SolveGrad(SolveMinProbl):
    ...
    def run(self,gamma=1e-5,Nit=15000,eps=1e-5):
        A = self.matr
        y = self.vect
...
if __name__ == '__main__':
    ...
    """ Data normalization """
    data_train_norm = copy.deepcopy(data_train) # To preserve oridinal data
    data_val_norm = copy.deepcopy(data_val) # To preserve oridinal data
    data_test_norm = copy.deepcopy(data_test) # To preserve oridinal data
    ...
    lls=SolveLLS(y_train.values,X_train.values,y_val.values,X_val.values)
    gd=SolveGrad(y_train.values,X_train.values,y_val.values,X_val.values)
    cgd=SolveConjugateGradient(y_train.values,X_train.values,y_val.values,X_val.values)
    sgd=SolveStochasticGradient(y_train.values,X_train.values,y_val.values,X_val.values)
    sd=SolveSteepestDescent(y_train.values,X_train.values,y_val.values,X_val.values)
    ridge=SolveRidge(y_train.values,X_train.values,y_val.values,X_val.values)
