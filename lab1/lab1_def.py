...
plt.rcParams.update({'figure.max_open_warning': 0})  # To avoid RAM usage warnings when displaying a lot of pictures
...
class SolveMinProbl(object):
    ...
    def plot_w(self, title, filename):
        """It plots (save on file, not showing) the w vector with stem and feature names on xlabel.
        """
        w = self.sol
        n = np.arange(self.Nf)
        plt.figure()
    ...
    def plot_err(self, title, filename, logy=1, logx=0):
        """It plots (save on file, not showing) the MSE in different log scales.
        Default: semilogy
    ...
    def graphics(self, title, filename):
        ...
        It plots (save on file, not showing) the histogram and scatter plot graphics.
...
class SolveRidge(SolveMinProbl):
    ...
    def run(self, Lambda=np.linspace(0.001,80,num=200)):
        ...
        for L in Lambda:
            ...
            self.errvalid.append(float(np.linalg.norm(np.dot(self.X_val,w)-self.y_val))**2/len(self.y_val))  # Error to find the best lambda
...
if __name__ == '__main__':
    ...
    #plt.show()  # If you want to display all the pictures
